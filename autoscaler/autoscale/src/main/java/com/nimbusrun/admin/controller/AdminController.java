...
public class AdminController {
  ...
  public ResponseEntity<ActionPoolStatus> nimbusConfig() {
    try {
      ActionPoolStatus s = new ActionPoolStatus(configReader.getBaseConfig().getComputeType(),
          githubApi.getRunnerGroupName(), compute.actionPoolToApiResponse());
  ...
  public ResponseEntity<List<WorkflowManagerResponse>> getPostedWebhooks() {
    try {
      return new ResponseEntity<>(this.adminService.getWorkflowManagers().stream()
          .map(WorkflowManagerResponse::fromWorkflowManager).toList(), HttpStatus.OK);
    } catch (Exception e) {
      log.error(e.getMessage(), e);
      return ResponseEntity.internalServerError().build();
    }
