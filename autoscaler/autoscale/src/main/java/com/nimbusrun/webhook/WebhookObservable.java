...
public class WebhookObservable {
        ...
        while (true) {
            try {
                ...
                while ((gj = this.githubActionJobs.poll(1, TimeUnit.MINUTES)) != null) {
                    log.debug("Evaluating Github Action Workflow Job for further processing %s".formatted(gj.simpleDescription()));
                    for (WebhookReceiver receiver : context.getBeansOfType(WebhookReceiver.class).values()) {
                        try {
                            receiver.receive(gj);
                        } catch (Exception e) {
                            log.error("Failed to to send github job payload to receiver %s".formatted(receiver.receiverName()), e);
                        }
                    }
                }
            } catch (Exception e) {
    ...
    /** Pass off the load to a list for later processing so that the http connection
     * with github can be closed as soon as possible
     *
     * @param message
     */
